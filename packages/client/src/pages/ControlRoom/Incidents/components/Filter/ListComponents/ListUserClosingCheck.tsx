...
export const ListUserClosingCheck = memo(
  ({ item, filterList, onFilter, disabled }: IListFilterComponents) => {
    ...
    useEffect(() => {
      const userClosingCheck = incidents
        .map(({ UserClosingCheck }) => {
          return {
            id: UserClosingCheck?.id,
            shortName: UserClosingCheck?.shortName,
          }
        })
        .filter(({ id }) => id)
      const _list = [...dispatchers, ...userClosingCheck]
      setList(
        _list
          .map(({ shortName, id }) => {
